...
namespace CassettaDiSicurezza
{
    public class CassettaSpeciale : CassettaSicurezza
    {
        ...
        {
            if (Oggetto == null) return 0;

            if (Oggetto is Gioiello gioiello) return gioiello.ValoreAssicurato * 0.9;

            if (Oggetto is DocumentoLegale documento) return documento.ValoreAssicurato * 0.8;

            if (Oggetto is Chiave chiave) return chiave.ValoreAssicurato * 0.7;
